...
#
# Reads:
#   sensor.day_ahead_price
#     - state = current price (float)
#     - attributes.records = list of {Time, End, Price}
#
# NEW:
# - Average calculation time window (hours) via input_number helper:
#     input_number.nordpool_avg_window_hours
#   This controls how many hours forward (from "now") are included in the average.
#   If helper is missing/invalid -> DEFAULT_AVG_WINDOW_HOURS is used.
#
...
#
# Writes per-unit bias helpers (input_number):
#   input_number.daikin1_price_bias_points
#
# Bias logic:
#   avg = mean(prices in window)
#   rel = (price_now - avg) / avg
#   if abs(rel_ema) < REL_DEADBAND -> bias = 0
#   else bias = clip( -rel_ema/REL_AT_MAX * MAX_BIAS_POINTS, -MAX_BIAS_POINTS, +MAX_BIAS_POINTS )
#
# Notes:
# - Negative bias => reduce demand when price is high.
# - Positive bias => increase demand when price is low.
# - This script DOES NOT change your Daikin demand directly.
#   It only writes helpers/sensors; your controller uses them.
...
# NEW: average window helper (hours)
...
MAX_AVG_WINDOW_HOURS = 48.0   # allow looking ahead up to 2 days
...
def _parse_iso_dt(s):
    # Accept '2025-12-22T00:00:00' and '2025-12-22T00:00:00+02:00' etc.
    if not s:
        return None
    try:
        ss = str(s)
        # If no timezone, assume local time; treat as naive and convert to timestamp via time.mktime-like logic
        # Simpler & robust: parse as naive and interpret as local time.
        # Python fromisoformat handles both.
...
def _in_window(record_time_dt, now_dt, window_hours):
    if record_time_dt is None:
        return False
    # If record is naive but now is aware (or vice versa), normalize by dropping tz
    if (record_time_dt.tzinfo is None) != (now_dt.tzinfo is None):
        record_time_dt = record_time_dt.replace(tzinfo=None)
...
def _update_debug_sensors(price_now, avg_price, rel_now, rel_ema, bias, total_count, in_window_count, window_hours):
    try:
        state.set(S_NOW, value=round(float(price_now), 6))
        state.set(S_AVG, value=round(float(avg_price), 6), window_hours=round(float(window_hours), 2), in_window=int(in_window_count), total=int(total_count))
        state.set(S_REL, value=round(float(rel_ema), 6), rel_now=round(float(rel_now), 6))
        state.set(S_BIAS, value=round(float(bias), 3), max_bias=MAX_BIAS_POINTS, rel_at_max=REL_AT_MAX, deadband=REL_DEADBAND)
...
def _update_nordpool_bias():
    ...
    if price_now is None:
        log.warning("Nordpool bias: %s state not a finite number: %s", PRICE_SENSOR, str(state.get(PRICE_SENSOR)))
        return
    ...
    prices, total_count, in_window_count = _extract_prices_from_records_window(records, now_dt, window_hours)
    avg_price = _calc_avg(prices)

    # Fallback if no records in window
    ...
    # Write per-unit helpers (same bias for all units)
    for unit, helper in UNIT_BIAS_HELPERS.items():
        _set_input_number(helper, bias)

    _update_debug_sensors(price_now, avg_price, rel_now, rel_ema, bias, total_count, in_window_count, window_hours)

    log.info(
        "Nordpool bias: win=%.2fh now=%.4f avg=%.4f rel_now=%.3f rel_ema=%.3f -> bias=%.2f pts (in_window=%d total=%d)",
        float(window_hours), float(price_now), float(avg_price),
...
def nordpool_bias_startup():
    # keep store persistent so EMA survives restarts
    try:
